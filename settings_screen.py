...
class SettingsScreen:
    def __init__(self, width, height, sound_manager):
        ...
        # Настройки
        self.settings = {
            'sound_volume': sound_manager.sound_volume if sound_manager else 0.8,
            'music_volume': sound_manager.music_volume if sound_manager else 0.7,
        }
        
        # Выбранный элемент меню
        self.selected_item = 0
        self.menu_items = ['Sound Volume', 'Music Volume', 'Back']
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_UP:
                ...
                if self.selected_item == len(self.menu_items) - 1:  # Back
                    return "MAIN_MENU"
    ...
    def adjust_setting(self, delta):
        ...
        if self.selected_item == 0:  # Sound Volume
            self.settings['sound_volume'] = max(0.0, min(1.0, 
                self.settings['sound_volume'] + delta))
            if self.sound_manager:
                self.sound_manager.set_sound_volume(self.settings['sound_volume'])
        elif self.selected_item == 1:  # Music Volume
            self.settings['music_volume'] = max(0.0, min(1.0, 
                self.settings['music_volume'] + delta))
            if self.sound_manager:
